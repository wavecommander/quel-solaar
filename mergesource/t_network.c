	...
	if(h != NULL)
	{
		if(port != 0)
		{
			...
			for(i = 0; i < port_count; i++)
			{
				...
				{
					to.port = 0;
					for(j = 0; j < testify_network_adapter_count(); j++)
					{
		...
		if(size = testify_network_receive(h, &from))
		{
			if(from.port ==  5351 && size >= 4)
			{
				...
				for(i = 0; i < testify_network_adapter_count() && from.ip != testify_network_adapter_ipv4_gateway(i); i++);
...
void testify_network_address_compare(TestifyNetworkAddress *address, char *dns_name)
{
}
