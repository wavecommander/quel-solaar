...
#define ADDRESS_MAX_NETWORK_ADDRES_STRING_SIZE 22
...
extern uint64	testify_pack_raw(THandle *handle, uint8 *data, uint64 lengt); /* Packs a raw array of bytes in to a handle. Returns the number of bytes that got packed as this may be less then size */
...
extern real64	testify_unpack_raw(THandle *handle, uint8 *buffer, uint64 buffer_length); /* Unpacks a raw array of bytes in to a handle. Returns the number of bytes that got unpacked as this may be less then size */
