...
void la_pfx_init(uint particle_count)
{
	...
	r_shader_state_set_depth_test(GlobalParticleData.spark_shader, FALSE); 
	r_shader_state_set_blend_mode(GlobalParticleData.spark_shader, 1, 1); /* Controls the shaders source and destinating blend factor. */
...
*/	}
	...
	r_shader_state_set_depth_test(GlobalParticleData.flare_shader, FALSE);
	r_shader_state_set_cull_face(GlobalParticleData.flare_shader, FALSE);
	r_shader_state_set_blend_mode(GlobalParticleData.flare_shader, GL_ONE, GL_ONE);
}
...
void lo_pfx_draw_local_space(BInputState *input)
{
	...
	if(input->mode == BAM_DRAW && la_axis_visible)
	{
		...
		seduce_object_3d_draw(input, 0, 0, 0, 0.6, SUI_3D_OBJECT_NORMAL, 1, &color[2]);	
		r_matrix_rotate(&la_world_matrix, -90, 0, 1, 0);
		seduce_object_3d_draw(input, 0, 0, 0, 0.6, SUI_3D_OBJECT_NORMAL, 1, &color[0]);	
		r_matrix_rotate(&la_world_matrix, 90, 0, 0, 1);
		seduce_object_3d_draw(input, 0, 0, 0, 0.6, SUI_3D_OBJECT_NORMAL, 1, &color[1]);	
...
void la_pfx_select_vertex(void)
{
	...
	udg_get_geometry(&vertex_count, NULL, &vertex, NULL, NULL);
	glDisable(GL_DEPTH_TEST);
//	dist = seduce_view_distance_camera_get();
//	dist *= 2;
