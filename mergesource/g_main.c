...
{
	if(session != NULL && GatherGlobal.plugins[session->plugin_type].image_stream_high_dynamic_range != NULL)
		return GatherGlobal.plugins[session->plugin_type].image_stream_high_dynamic_range(session->plugin_session, stream_id);
	return FALSE;
}
...
void *gather_image_load(char *file_name, uint *x, uint *y, boolean *hdr)
{
	...
	if(0 < gather_session_image_stream_count_get(session))
	{
		...
		*hdr = gather_session_image_stream_high_dynamic_range(session, 0);
		...
		if(data != NULL)
		{	
			copy = malloc(size * *x * *y * 4);
			if(flip)
			{
				for(i = 0; i < *y; i++)
					memcpy(&copy[size * *x * 4 * (*y - i - 1)], &data[size * *x * 4 * i], size * *x * 4);
			}else
			{
				copy = malloc(size * *x * *y * 4);
				memcpy(copy, data, size * *x * *y * 4);
			}
		}
	}
