...
"	vec4 v;"
"	fade = select.gb;"
"	col = select_color * select.rrrr + crease_color * select.aaaa;"
"	v = vec4(vertex.xyz, 1.0);"
...
void la_do_init(void)
{
	...
	r_shader_state_set_blend_mode(GlobalOverlay.line_shader, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
	r_shader_state_set_blend_mode(GlobalOverlay.line_shader, GL_ONE, GL_ONE);
	r_shader_state_set_depth_test(GlobalOverlay.line_shader, GL_ALWAYS);
	r_shader_vec4_set(NULL, r_shader_uniform_location(GlobalOverlay.line_shader, "base_color"), 0.6, 1.6, 2.0, 0.15);
	r_shader_vec4_set(NULL, r_shader_uniform_location(GlobalOverlay.line_shader, "fog_color"), 0.1, 0.0, 0.1, 0.15);
	r_shader_vec4_set(NULL, r_shader_uniform_location(GlobalOverlay.line_shader, "select_color"), 0.6, 0.4, 0.1, 0.5);
	r_shader_vec4_set(NULL, r_shader_uniform_location(GlobalOverlay.line_shader, "crease_color"), 0.5, 0.3, 0.8, 0.5);
	...
	r_shader_state_set_blend_mode(GlobalOverlay.poly_shader, GL_ONE, GL_ONE);
	r_shader_state_set_depth_test(GlobalOverlay.poly_shader, GL_ALWAYS);
	...
	r_shader_state_set_blend_mode(GlobalOverlay.vertex_shader, GL_ONE, GL_ONE);
	r_shader_state_set_depth_test(GlobalOverlay.vertex_shader, FALSE);
	...
	r_shader_state_set_blend_mode(GlobalOverlay.delete_shader, GL_ONE, GL_ONE);
	r_shader_state_set_depth_test(GlobalOverlay.delete_shader, FALSE);
	...
	r_shader_state_set_blend_mode(GlobalOverlay.base_shader, GL_ONE, GL_ONE);
	r_shader_state_set_depth_test(GlobalOverlay.base_shader, FALSE);
}
...
void la_do_active_vertex(BInputState *input, double *vertex, boolean move)
{
	...
	if(move)
		seduce_object_3d_draw(input, pos[0], pos[1], 0, 0.016, SUI_3D_OBJECT_MOVE_3D, 1, color);
	else
		seduce_object_3d_draw(input, pos[0], pos[1], 0, 0.016, SUI_3D_OBJECT_SELECT, 1, color);
}
					...
					if(temp[3] < length * 3 && vertex[temp[3]] != V_REAL64_MAX)
					{
						...
						*poly++ = s;
						...
						if(crease != NULL)
							...
							c = 0;
				/*		if(select != NULL)
							f = select[ref[i]] * select[ref[i + 1]] * s;
						else
							f = 0;*/
...
void la_do_mesh_surface(LoOverlay *overlay)
{
	...
	{
		r_shader_set(GlobalOverlay.line_shader);
		r_shader_float_set(GlobalOverlay.line_shader, r_shader_uniform_location(GlobalOverlay.line_shader, "distance"), 10.0 / seduce_view_distance_camera_get(NULL));
