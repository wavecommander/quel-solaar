...
extern float	seduce_text_line_draw(SeduceRenderFont *font, float pos_x, float pos_y, float letter_size, float letter_spacing, const char *text, float red, float green, float blue, float alpha, uint length); /* Draws a like of text. */
...
extern boolean seduce_widget_slider_line(BInputState *input, void *id, float *value, float pos_a_x, float pos_a_y, float pos_b_x, float pos_b_y, float size, float scale, float min, float max, float time, float *color, boolean snaps); /* Creates a linear slider between two points. The snaps option enables rulers for precise values. */
extern boolean seduce_widget_slider_radial(BInputState *input, void *id, float *value, float pos_x, float pos_y, float size, float scale, float min, float max, float time, float *color); /* Creates a radial slider. */ 
extern boolean seduce_widget_slider_radius(BInputState *input, void *id, float *value, float pos_x, float pos_y, float size, float time, float *color); /* Creates a radius slider. (Useful for things like brush sizes.) */
extern boolean seduce_widget_color_wheel_radial(BInputState *input, void *id, float *color, float pos_x, float pos_y, float size, float scale, float time); /* Cretes a color wheel with Hue and Saturation */
extern boolean seduce_widget_color_triangle_radial(BInputState *input, void *id, float *color, float pos_x, float pos_y, float size, float scale, float time); /* Creates a color trangle with Value and saturation. */
extern boolean seduce_widget_color_square_radial(BInputState *input, void *id, float *color, uint component, float pos_x, float pos_y, float size, float scale, float time); /* creates a color square that is an intersection of the RGB cube at either R, G or B component.*/
...
extern boolean	seduce_manipulator_point(BInputState *input, RMatrix *m, float *pos, void *id, float scale); /* Creates a manipulator for moving a point in space. */
extern boolean	seduce_manipulator_pos_xyz(BInputState *input, RMatrix *m, float *pos, void *id, float *snap, boolean snap_active, boolean x, boolean y, boolean z, float scale, float time); /* Creates a manipulator for moving a point in space using 3 separate handles representing the X, Y and Z axis. */
extern boolean	seduce_manipulator_point_plane(BInputState *input, RMatrix *m, float *pos, void *id, float *snap, boolean snap_active, uint axis, float scale); /* Creates a manipulator for moving a point in 2 out of the 3 dimentions.*/
extern boolean	seduce_manipulator_point_axis(BInputState *input, RMatrix *m, float *pos, void *id, float *snap, boolean snap_active, uint axis, float scale);/* Creates a manipulator for moving a point along one axis in 3 dimentions.*/
extern boolean	seduce_manipulator_point_vector(BInputState *input, RMatrix *m, float *pos, void *id, float *snap, boolean snap_active, float *vector, float scale);/* Creates a manipulator for moving a point along a vector in 3 dimentions.*/

extern boolean	seduce_manipulator_normal(BInputState *input, RMatrix *m, float *pos, float *normal, void *id, float scale, float time); /* Creates a manipulator for defining a normalized vector in 3D space. */
extern boolean	seduce_manipulator_radius(BInputState *input, RMatrix *m, float *pos, float *radius, void *id, float time);/* Creates a manipulator for defining a radius in 3D space. */
extern boolean	seduce_manipulator_scale(BInputState *input, RMatrix *m, float *pos, float *size, void *id, float *snap, boolean snap_active, boolean x, boolean y, boolean z, float scale, float time);/* Creates a manipulator for scaling 3 size values in 1, 2 or 3 axis. */
extern boolean	seduce_manipulator_rotate(BInputState *input, RMatrix *matrix, float *pos, float *rotation_matrix, void *id, float *snap, boolean snap_active, float scale, float time);/* Creates a manipulator for 3D rotation of a 4x4 matrix. */
...
extern boolean	seduce_manipulator_slider(BInputState *input, void *id, float *scroll, float *pos_a, float *pos_b, float full_length, float section_length, float size, float red, float green, float blue);/* Creates slider between 2 points in 3d space. */
