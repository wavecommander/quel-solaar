...
{
	FILE *file;
	file = fopen("OpenNI_error.txt", "w");
	openni::Status rc = openni::STATUS_OK;
	const char* deviceURI = openni::ANY_DEVICE;
//	int i = STREAM_PROPERTY_VERTICAL_FOV;

	rc = openni::OpenNI::initialize();

	fprintf(file, "After initialization:\n%s\n", openni::OpenNI::getExtendedError());

	rc = context.device.open(deviceURI);
	if (rc != openni::STATUS_OK)
	{
		fprintf(file, "SimpleViewer: Device open failed:\n%s\n", openni::OpenNI::getExtendedError());
	...
	if (rc == openni::STATUS_OK)
	{
		...
		if (rc != openni::STATUS_OK)
		{
			fprintf(file, "SimpleViewer: Couldn't start depth stream:\n%s\n", openni::OpenNI::getExtendedError());
			context.depth_stream.destroy();
		}
	}
	else
	{
		fprintf(file, "SimpleViewer: Couldn't find depth stream:\n%s\n", openni::OpenNI::getExtendedError());
	}
	...
	if (rc == openni::STATUS_OK)
	{
		...
		if (rc != openni::STATUS_OK)
		{
			fprintf(file, "SimpleViewer: Couldn't start color stream:\n%s\n", openni::OpenNI::getExtendedError());
			context.color_stream.destroy();
		}
	}
	else
	{
		fprintf(file, "SimpleViewer: Couldn't find color stream:\n%s\n", openni::OpenNI::getExtendedError());
	}

	if (!context.depth_stream.isValid() || !context.color_stream.isValid())
	{
		fprintf(file, "SimpleViewer: No valid streams. Exiting\n");
		openni::OpenNI::shutdown();
		return 2;
	}
	...
	context.color_size[0] = context.colorVideoMode.getResolutionX();
	context.color_size[1] = context.colorVideoMode.getResolutionY();
	fprintf(file, "Depth %u\n", context.depthVideoMode.getResolutionX());
	fprintf(file, "Depth %u\n", context.depthVideoMode.getResolutionY());
	fprintf(file, "color %u\n", context.colorVideoMode.getResolutionX());
...
unsigned short *OpenNI_camera_depth_read(unsigned int *x, unsigned int *y)
{
	openni::VideoFrameRef frame;
	openni::DepthPixel* depth_buffer;
...
unsigned short *OpenNI_camera_color_read(unsigned int *x, unsigned int *y)
{
	openni::VideoFrameRef frame;
	openni::RGB888Pixel* color_buffer;

	context.color_stream.readFrame(&frame); 
	color_buffer = (openni::RGB888Pixel*)frame.getData();
	*x = context.depth_size[0];
	*y = context.depth_size[1];
	return (unsigned short *)color_buffer;
}
#ifdef __cplusplus
} // extern "C"
#endif