...
"	ref = normalize(reflect(camera_vec, n2));"
"	if(ref.y < 0.0)"
"	{"			
"		gl_FragColor += mix(vec4(0.08, 0.24, 0.4, 0), vec4(0.02, 0.05, 0.1, 0), -ref.y) * vec4(freznel);"
"	}"
"}";
...
boolean draw_persuade_surface_queue_preview(ENode *node)
{
	...
	for(j = 0; j < LA_PREVIEW_COUNT; j++)
	{
		if(lo_preview_working[j] == LA_OPS_EMPTY)
		{
			...
			imagine_thread(lo_overlay_surface_func, &lo_preview_surface[j]);	
...
void draw_persuade_surface(BInputState *input)
{
	...
	if(!working)
	{
		...
		{	
			...
			if(lo_overlay_surface.ref_count < limit)
			{
				...
				imagine_thread(lo_overlay_surface_func, &lo_overlay_surface);	
				working = TRUE;
				structure_version = v;
	...
	}else
	{
		...
		if(lo_pointer_polygon < ref_count)
		{
