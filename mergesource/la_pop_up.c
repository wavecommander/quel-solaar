...
boolean la_pu_object_selection(BInputState *input, boolean active)
{
	...
	if(seduce_widget_button_icon(input, &time, SUI_3D_OBJECT_CLOSE, 1 - 0.025, -aspect + 0.025,  0.025, time, color))
	{		
...
LAPUEmptyReturn la_pu_empty(BInputState *input, boolean active)
{
	...
	if((layer_count + 9) > element_allocations)
	{
		element_allocations = (layer_count + 9);
		if(element != NULL)
			free(element);
	...
	element[4].text = seduce_translate("Rename");
	element[5].type = S_PUT_TOP;
	element[5].text = seduce_translate("New");


	element[6].type = S_PUT_ANGLE;
	element[6].text = seduce_translate("Undo");
	element[6].data.angle[0] = 45;
	element[6].data.angle[1] = 135;
	element[7].type = S_PUT_ANGLE;
	element[7].text = seduce_translate("Redo");
	element[7].data.angle[0] = 225;
	element[7].data.angle[1] = 315;
	element[8].type = S_PUT_ANGLE;
	element[8].text = seduce_translate("Select Geometry");
	element[8].data.angle[0] = 135;
	element[8].data.angle[1] = 225;
	count = 9;
	i = 0;
	...
	ring = seduce_popup(&fake_input, element, element, count, time);
	free(pointers);
	if(input->mode == BAM_DRAW)	
		r_matrix_pop(NULL);
	switch(ring)
...
boolean la_pu_manipulator(BInputState *input, boolean active)
{
	...
	element[8].text = seduce_translate("Delete");
	...
	ring = seduce_popup(&fake_input, element, element, 29, time);
	free(pointers);
	if(input->mode == BAM_DRAW)	
		r_matrix_pop(NULL);
...
boolean la_pu_edge(BInputState *input, uint *edge, boolean active)
{
	...
	ring = seduce_popup(&fake_input, element, element, 10, time);
	free(pointers);
	if(input->mode == BAM_DRAW)	
		r_matrix_pop(NULL);
	if(active)
...
boolean la_pu_polygon(BInputState *input, uint polygon, boolean active)
{
	...
	ring = seduce_popup(&fake_input, element, element, 11, time);
	free(pointers);
	if(input->mode == BAM_DRAW)	
		r_matrix_pop(NULL);
	if(active)
...
uint la_pu_select(BInputState *input, boolean active)
{
	...
	ring = seduce_popup(&fake_input, element, element, 4, time);
	free(pointers);
	if(input->mode == BAM_DRAW)
		r_matrix_pop(NULL);
	return ring;
