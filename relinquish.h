...
extern boolean r_init(void *(*glGetProcAddr)(const char* proc)); /* r_init initializes Relinquish and returns TRUE if it sucseeds and FALSE if the hardware does not support the required features. As of right now the Reliquish requires  GL_ARB_shading_language_100 and  GL_ARB_framebuffer_object. The funtion requires a function pointer to the systems procAddressGet in order to be able to obtain extention pointers */

extern void r_enable(); /* Enables Reliquish to take over all the state of the underlying API. */
extern void r_disable(); /* Disables Reliquish hold on the underlying API. */
...
extern boolean r_resource_debug();
...
extern uint r_extension_query(uint query);
...
RFormats vertex_format_types[3] = {R_FLOAT, R_UINT16, R_UINT8};
...
extern uint 	r_array_vertex_size(RFormats *vertex_format_types, uint *vertex_format_size, uint vertex_format_count); /* computes size in bytes of a specific vertext format. */
...
extern void 	*r_array_allocate(uint vertex_count, RFormats *vertex_format_types, uint *vertex_format_size, uint vertex_format_count, uint reference_count); /* Allocates a pool of vertex_count number of vertices */
...
extern void		r_matrix_cube_map(RMatrix *matrix, uint side); /* Rotates the camera in one of 6 directions used for qubemap rendering. */
...
extern void r_matrix_projection_camerad(RMatrix *matrix, double *output, double x, double y, double z);
extern void r_matrix_projection_cameraf(RMatrix *matrix, float *output, float x, float y, float z);
...
extern void r_viewport(int x_start, int y_start, int x_end, int y_end); /* sets, in pixes the view port where Relinquish will draw.*/
...
extern void		r_shader_state_set_stencil(RShader *shader, boolean front, RStencilFunc function, uint32 reference, uint32 mask, RStencilOp stencil_fail_op, RStencilOp depth_fail_op, RStencilOp sucsess_op); /* Controms the shaders chanel mask where the rendering of individual components can be turned on and off.*/ 
...
extern void		r_shader_debug_print_shader(RShader *shader);/*Prints out of stdout the currents state of the shader including, name its inputs and outputs, uniforms and uniform blocks as will as the current uniform state.*/
...
extern void		r_texture_update(uint texture_id, PImageFormat format, uint x_offset, uint y_offset, uint z_offset, uint x_size, uint y_size, uint z_size, void *data); /* update the content pixels of a texture */
...
#define RELINQUISH_TARGET_CUBE_MAP_POSITIVE_X_EXT  0 
#define RELINQUISH_TARGET_CUBE_MAP_NEGATIVE_X_EXT  1 
#define RELINQUISH_TARGET_CUBE_MAP_POSITIVE_Y_EXT  2 
#define RELINQUISH_TARGET_CUBE_MAP_NEGATIVE_Y_EXT  3 
#define RELINQUISH_TARGET_CUBE_MAP_POSITIVE_Z_EXT  4 
#define RELINQUISH_TARGET_CUBE_MAP_NEGATIVE_Z_EXT  5 
#define RELINQUISH_TARGET_2D_TEXTURE -1 

extern void 	*r_framebuffer_allocate(uint *buffers, uint buffer_count, uint depth_buffer, uint target); /* Creates a FBO form an array of textures and/or a depth/stencil buffer. All buffers has to be of the same size. it will return -1 if the different texture and depth formats are not renderable or compatible.*/
extern void		r_framebuffer_free(void *fbo); /*Free a Frame buffer object. */
extern void		r_framebuffer_bind(void *id); /* Binds a frame buffer object*/
extern uint		r_framebuffer_id(void *fbo); /* extract frame buffer id */
