...
extern boolean b_win32_init_display_opengl(uint size_x, uint size_y, boolean full_screen, uint samples, char *caption, boolean *sterioscopic);
...
boolean betray_screen_mode_set(uint x_size, uint y_size, uint samples, boolean fullscreen)
{
	...
	if(betray_context_type == B_CT_OPENGL)
		if(!b_win32_init_display_opengl(x_size, y_size, fullscreen, samples, BGlobal.caption, &BGlobal.sterioscopic))
			return FALSE;
	#endif
...
uint betray_audio_sound_play(uint sound, float *pos, float *vector, float speed, float volume, boolean loop, boolean ambient, boolean auto_delete)
{
	...
	}else
	{
		...
		if(id == BGlobal.sounds.source_allocated)
			BGlobal.sounds.source_ids = (uint *)realloc(BGlobal.sounds.source_ids, (sizeof *BGlobal.sounds.source_ids) * (BGlobal.plugins.audio_count + 1));
...
uint betray_plugin_pointer_allocate(uint user_id, uint button_count, float x, float y, char *name)
{
	uint i, code;
...
void betray_plugin_pointer_set(uint id, float x, float y, boolean *buttons)
{
	uint i;
	if(id >= 256)
		return;
...
uint betray_plugin_axis_allocate(uint user_id, char *name, BAxisType type, uint axis_count)
{
	uint i, id;
...
//	betray_context_type = context_type;
	...
	betray_plugin_pointer_allocate(0, 3, 0.0, 0.0, "mouse");

	BGlobal.input.axis_count = 0;
	BGlobal.input.axis = NULL;
	BGlobal.input.user_count = 2;
	...
	BGlobal.input.frame_number = 0;
		...
		{
			if(!b_win32_init_display_opengl(window_size_x, window_size_y, window_fullscreen,  samples, name, &BGlobal.sterioscopic))
			{
			#ifdef BETRAY_CONTEXT_OPENGLES
