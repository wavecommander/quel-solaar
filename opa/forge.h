...
extern boolean f_debug_mem_comment(void *buf, char *comment); /* add a comment to am allocation that can help identyfy its use. */
...
#define f_debug_memory_init(n, m)
...
extern char		*f_text_load(char *file_name);

/* ---- Sorting ------ 
A fast Merge sort implementations for sorting integers and pointers */

extern uint		*f_sort_ids(uint length, uint *ids, boolean (*compate_func)(uint bigger, uint smaller, void *user), void *user);
extern void		**f_sort_pointers(uint length, void **pointers, boolean (*compate_func)(void *bigger, void *smaller, void *user), void *user);
