	...
	if(input->mode == BAM_DRAW)
	{
		...
		if(element_count < 9)
		{
			...
			if(angle == 0)
			{
				...
				if(active)
					f = 1.0;
				seduce_text_line_draw(NULL, seduce_text_line_length(SEDUCE_T_SIZE * -0.75 * (3.0 - f * 2.0), SEDUCE_T_SPACE, lables[0], -1), 0.15 - SEDUCE_T_SIZE, SEDUCE_T_SIZE * 1.5 * (3.0 - f * 2.0), SEDUCE_T_SPACE, lables[0], red_active, green_active, blue_active, *time, -1);
			}else
			{
				...
				seduce_text_line_draw(NULL, seduce_text_line_length(SEDUCE_T_SIZE * -0.5, SEDUCE_T_SPACE, lables[0], -1), 0.15 - SEDUCE_T_SIZE, SEDUCE_T_SIZE, SEDUCE_T_SPACE, lables[0], red, green, blue, *time, -1);
			}
			r_matrix_pop(&matrix);
			i = 1;
		}else
		...
		if(element_count < 9 && element_count % 2 == 0)
		{
			...
			if(angle == i)
			{
				...
				if(active)
					f = 1.0;
				seduce_text_line_draw(NULL, seduce_text_line_length(SEDUCE_T_SIZE * -0.75 * (3.0 - f * 2.0), SEDUCE_T_SPACE, lables[i], -1), -0.15, SEDUCE_T_SIZE * 1.5 * (3.0 - f * 2.0), SEDUCE_T_SPACE, lables[i], red_active, green_active, blue_active, f, -1);
			}else
			{
				r_matrix_rotate(&matrix, 180 + 360 * (float)i / (float)element_count, 0, 0, 1);
				seduce_text_line_draw(NULL, seduce_text_line_length(SEDUCE_T_SIZE * -0.5, SEDUCE_T_SPACE, lables[i], -1), -0.15, SEDUCE_T_SIZE, SEDUCE_T_SPACE, lables[i], red, green, blue, *time, -1);
			}
			r_matrix_pop(&matrix);
			i++;
		}
		for(; i < element_count; i++)
		{
			...
			if(angle == i)
			{
				...
				if(active)
					f = 1.0;
				seduce_text_line_draw(NULL, -0.1 - seduce_text_line_length(SEDUCE_T_SIZE * 1.5 * (3.0 - f * 2.0), SEDUCE_T_SPACE, lables[i], -1), 0.0, SEDUCE_T_SIZE * 1.5 * (3.0 - f * 2.0), SEDUCE_T_SPACE, lables[i], red_active, green_active, blue_active, f, -1);

			}else
			{
				r_matrix_rotate(&matrix, -90 - 360 * (float)i / (float)element_count, 0, 0, 1);
				seduce_text_line_draw(NULL, -0.1 - seduce_text_line_length(SEDUCE_T_SIZE, SEDUCE_T_SPACE, lables[i], -1), 0.0, SEDUCE_T_SIZE, SEDUCE_T_SPACE, lables[i], red, green, blue, *time, -1);
...
boolean seduce_popup_top_bottom_text_element(BInputState *input, void *id, uint element_nr, char *text, float time,  boolean top)
{
	...
	if(top)
	{
		seduce_text_line_draw(NULL, -seduce_text_line_length(SEDUCE_T_SIZE * 0.5, SEDUCE_T_SPACE, text, -1), 0.1 * vec + vec * 0.005 + (SEDUCE_T_SIZE * 2.0) + (SEDUCE_T_SIZE * 5.0) * (float)element_nr * time * time, SEDUCE_T_SIZE, SEDUCE_T_SPACE, text, 0.0, 0.0, 0.0, time * time, -1);
	}else
		seduce_text_line_draw(NULL, -seduce_text_line_length(SEDUCE_T_SIZE * 0.5, SEDUCE_T_SPACE, text, -1), -0.1 * vec - vec * 0.005 + (SEDUCE_T_SIZE * -3.0) + (SEDUCE_T_SIZE * -5.0) * (float)element_nr * time * time, SEDUCE_T_SIZE, SEDUCE_T_SPACE, text, 0.0, 0.0, 0.0, time * time, -1);
    return FALSE;
}
...
boolean seduce_text_angle(BInputState *input, void *id, float pos_x, float pos_y, float angle_a, float angle_b, char *text, float time)
{
	...
	}else
	{
		...
		seduce_text_line_draw(NULL, -0.12 - seduce_text_line_length(SEDUCE_T_SIZE, SEDUCE_T_SPACE, text, -1), 0, SEDUCE_T_SIZE, SEDUCE_T_SPACE, text, 0.0, 0.0, 0.0, time * time, -1);
	}
...
STypeInState seduce_popup_text(BInputState *input, void *id, uint *selected, char **lables, uint element_count, SPopUpType type, float pos_x, float pos_y, float center, float size, float spacing, const char *text, float red, float green, float blue, float alpha, float red_select, float green_select, float blue_select, float alpha_select, boolean release_only)
{
	...
	return seduce_widget_select_radial(input, id, selected, lables, element_count, type,  pos_x, pos_y, size, 0.0, 1, release_only);
}
