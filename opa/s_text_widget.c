...
extern void		sui_text_line_edit_draw(BInputState *input, float pos_x, float pos_y, float size, float spacing, float length, char *text, uint *scroll_start, uint select_start, uint select_end, float red, float green, float blue, float alpha);
...
extern void sui_text_edit_up(char *text, uint *select_start, uint *select_end);
extern void sui_text_edit_down(char *text, uint *select_start, uint *select_end);
...
extern void sui_text_edit_delete(char *text, uint length, uint *select_start, uint *select_end, STextBlockMode *modes, uint mode_count);
...
extern void sui_text_edit_backspace(char *text, uint length, uint *select_start, uint *select_end, STextBlockMode *modes, uint mode_count);
extern void sui_text_edit_insert_character(char *text, uint length, uint *select_start, uint *select_end, uint32 character, STextBlockMode *modes, uint mode_count);
extern void sui_text_edit_paste(char *text, uint length, uint *select_start, uint *select_end, STextBlockMode *modes, uint mode_count);
extern void sui_text_edit_cut(char *text, uint length, uint *select_start, uint *select_end, STextBlockMode *modes, uint mode_count);
...
boolean seduce_text_button(BInputState *input, void *id, float pos_x, float pos_y, float center, float size, float spacing, const char *text, float red, float green, float blue, float alpha, float red_select, float green_select, float blue_select, float alpha_select)
{
	...
	if(input->mode == BAM_DRAW)
	{
		...
		if(acticve)
			length = seduce_text_line_draw(NULL, a[0], a[1], size, spacing, text, red_select, green_select, blue_select, alpha_select, -1);
		else
			length = seduce_text_line_draw(NULL, a[0], a[1], size, spacing, text, red, green, blue, alpha, -1);
		b[0] = a[0] + length;
		a[1] += size * 0.5;
		b[1] += size * 0.5;
...
boolean seduce_text_active(void *id)
{
	uint i, count;
	count = betray_support_functionality(B_SF_USER_COUNT_MAX);
	for(i = 0; i < count; i++)
...
// uint seduce_text_block_draw3(float pos_x, float pos_y, float line_size, float height, float line_spacing, SUIBlockStyle style, const char *text, uint pos, STextBlockMode *modes, uint mode_count)
/*
typedef struct{
	float pos_x;
	float pos_y;
...
extern void seduce_text_block_select_draw(BInputState *input, float pos_x, float pos_y, float line_size, float height, float line_spacing, SUIBlockStyle style, const char *text, uint pos, STextBlockMode *modes, uint mode_count, uint select_start, uint select_end);
...
//		seduce_element_add_line(input, id, 0, array, &array[3], size * 2.0);
	...
	//			seduce_text_line_draw(NULL, pos_x, pos_y, size, SEDUCE_T_SPACE, &t[seduce_text_state[i].scroll_start], active_red, active_green, active_blue, active_alpha, seduce_text_line_hit_test(size, SEDUCE_T_SPACE, &t[seduce_text_state[i].scroll_start], length) + seduce_text_state[i].scroll_start);
	//			sui_text_line_edit_draw(input, pos_x, pos_y, size, SEDUCE_T_SPACE, length, t, &seduce_text_state[i].scroll_start, seduce_text_state[i].select_start, seduce_text_state[i].select_end, active_red, active_green, active_blue, active_alpha);
				break;
			}
		}
				...
				for(j = 0; j < input->button_event_count; j++)
				{
					if(input->button_event[j].state == TRUE && input->button_event[j].user_id == i)
					{
						...
						if(input->button_event[j].button == BETRAY_BUTTON_PASTE)
							sui_text_edit_paste(t, buffer_size, &seduce_text_state[i].select_start, &seduce_text_state[i].select_end, modes, mode_count);
						else if(input->button_event[j].button == BETRAY_BUTTON_CUT)
							sui_text_edit_cut(t, buffer_size, &seduce_text_state[i].select_start, &seduce_text_state[i].select_end, modes, mode_count);
							...
							sui_text_edit_delete(t, buffer_size, &seduce_text_state[i].select_start, &seduce_text_state[i].select_end, modes, mode_count);
							...
							sui_text_edit_backspace(t, buffer_size, &seduce_text_state[i].select_start, &seduce_text_state[i].select_end, modes, mode_count);
						else if(input->button_event[j].character > 31 && input->button_event[j].character < 256)
							sui_text_edit_insert_character(t, buffer_size, &seduce_text_state[i].select_start, &seduce_text_state[i].select_end, input->button_event[j].character, modes, mode_count);
					}
				}
			}
		}
					...
					/*	if(input->button_event[j].button == BETRAY_BUTTON_SCROLL_DOWN)
						...
						}else*/
						{
							if(input->button_event[j].button == BETRAY_BUTTON_PASTE)
								...
								sui_text_edit_up(t, &seduce_text_state[i].select_start, &seduce_text_state[i].select_end);
							else if(input->button_event[j].button == BETRAY_BUTTON_DOWN)
								sui_text_edit_down(t, &seduce_text_state[i].select_start, &seduce_text_state[i].select_end);
							else if(input->button_event[j].button == BETRAY_BUTTON_RIGHT)
								sui_text_edit_forward(t, &seduce_text_state[i].select_start, &seduce_text_state[i].select_end);
							else if(input->button_event[j].button == BETRAY_BUTTON_LEFT)
								sui_text_edit_back(t, &seduce_text_state[i].select_start, &seduce_text_state[i].select_end);
								...
								sui_text_edit_insert_character(t, buffer_size, &seduce_text_state[i].select_start, &seduce_text_state[i].select_end, '\t', modes, mode_count);
							else if(input->button_event[j].button == BETRAY_BUTTON_RETURN)
								sui_text_edit_insert_character(t, buffer_size, &seduce_text_state[i].select_start, &seduce_text_state[i].select_end, '\n', modes, mode_count);
							else if(input->button_event[j].character > 31 && input->button_event[j].character < 256)
								sui_text_edit_insert_character(t, buffer_size, &seduce_text_state[i].select_start, &seduce_text_state[i].select_end, input->button_event[j].character, modes, mode_count);
							if(seduce_text_state[i].select_start == seduce_text_state[i].select_end)
								sui_text_monospace_edit_scroll(text, line_count, line_length, scroll, seduce_text_state[i].select_start);
						}
					}
				}
			}
...
STypeInState seduce_text_line_edit(BInputState *input, void *id, char *text, uint buffer_size, float pos_x, float pos_y, float length, float size, char *label, boolean left, void (*done_func)(void *user, char *text), void *user, float red, float green, float blue, float alpha, float active_red, float active_green, float active_blue, float active_alpha)
{
	...
	if(input->mode == BAM_DRAW)
	{
		...
		if(i == input->user_count)
		{
			...
			if(i < input->user_count)
				seduce_text_line_draw(NULL, pos_x, pos_y, size, SEDUCE_T_SPACE, t, active_red, active_green, active_blue, active_alpha, seduce_text_line_hit_test(size, SEDUCE_T_SPACE, t, length));
			else
				seduce_text_line_draw(NULL, pos_x, pos_y, size, SEDUCE_T_SPACE, t, red, green, blue, alpha, seduce_text_line_hit_test(size, SEDUCE_T_SPACE, t, length));
		}
	}else if(input->mode == BAM_EVENT)
	{
		for(i = 0; i < input->pointer_count; i++)
				...
				for(j = 0; j < input->button_event_count; j++)
				{
					if(input->button_event[j].state == TRUE && input->button_event[j].user_id == i)
					{
						if(seduce_text_state[i].done_func != NULL)
							...
							t = text;
						if(input->button_event[j].button == BETRAY_BUTTON_PASTE)
							sui_text_edit_paste(t, buffer_size, &seduce_text_state[i].select_start, &seduce_text_state[i].select_end, NULL, 0);
						else if(input->button_event[j].button == BETRAY_BUTTON_CUT)
							sui_text_edit_cut(t, buffer_size, &seduce_text_state[i].select_start, &seduce_text_state[i].select_end, NULL, 0);
						else if(input->button_event[j].button == BETRAY_BUTTON_COPY)
							sui_text_edit_copy(t, buffer_size, &seduce_text_state[i].select_start, &seduce_text_state[i].select_end);
						else if(input->button_event[j].button == BETRAY_BUTTON_RIGHT)
							sui_text_edit_forward(t, &seduce_text_state[i].select_start, &seduce_text_state[i].select_end);
							...
							sui_text_edit_delete(t, buffer_size, &seduce_text_state[i].select_start, &seduce_text_state[i].select_end, NULL, 0);
						else if(input->button_event[j].button == BETRAY_BUTTON_END)
							sui_text_edit_end(t, buffer_size, &seduce_text_state[i].select_start, &seduce_text_state[i].select_end);
						else if(input->button_event[j].button == BETRAY_BUTTON_HOME)
							sui_text_edit_home(t, buffer_size, &seduce_text_state[i].select_start, &seduce_text_state[i].select_end);
						else if(input->button_event[j].button == BETRAY_BUTTON_BACKSPACE)
							sui_text_edit_backspace(t, buffer_size, &seduce_text_state[i].select_start, &seduce_text_state[i].select_end, NULL, 0);
						else if(input->button_event[j].button == BETRAY_BUTTON_RETURN) 
						{
							if(seduce_text_state[i].done_func != NULL)
							{
							...
							sui_text_edit_insert_character(t, buffer_size, &seduce_text_state[i].select_start, &seduce_text_state[i].select_end, input->button_event[j].character, NULL, 0);
					}
				}
			}
		}
		...
		for(i = 0; i < input->user_count; i++)
		{
			if(seduce_text_state[i].id == id)
			{
				...
				for(j = 0; t[j] != 0; j++)
					password[j] = '*';
				password[j] = 0;
				seduce_text_line_draw(NULL, pos_x, pos_y, size, SEDUCE_T_SPACE, &password[seduce_text_state[i].scroll_start], active_red, active_green, active_blue, active_alpha, seduce_text_line_hit_test(size, SEDUCE_T_SPACE, &t[seduce_text_state[i].scroll_start], length) + seduce_text_state[i].scroll_start);
				sui_text_line_edit_draw(input, pos_x, pos_y, size, SEDUCE_T_SPACE, length, password, &seduce_text_state[i].scroll_start, seduce_text_state[i].select_start, seduce_text_state[i].select_end, active_red, active_green, active_blue, active_alpha);
				free(password);
				break;
			}
		}
...
STypeInState seduce_text_double_edit(BInputState *input, void *id, double *value, float pos_x, float pos_y, float length, float size, boolean left, void (*done_func)(void *user, double value), void *user, float red, float green, float blue, float alpha, float active_red, float active_green, float active_blue, float active_alpha)
{
	...
	if(input->mode == BAM_DRAW)
	{
		...
		if(i == input->user_count)
		{
			...
			for(i = 0; i < input->user_count && id != seduce_element_pointer_id(input, i, NULL); i++);
			if(i < input->user_count)
				seduce_text_line_draw(NULL, pos_x, pos_y, size, SEDUCE_T_SPACE, buffer, active_red, active_green, active_blue, active_alpha, seduce_text_line_hit_test(size, SEDUCE_T_SPACE, buffer, length));
			else
				seduce_text_line_draw(NULL, pos_x, pos_y, size, SEDUCE_T_SPACE, buffer, red, green, blue, alpha, seduce_text_line_hit_test(size, SEDUCE_T_SPACE, buffer, length));
		}
				...
				for(j = 0; j < input->button_event_count; j++)
				{
					if(input->button_event[j].state == TRUE && input->button_event[j].user_id == i)
					{
						...
						if(input->button_event[j].button == BETRAY_BUTTON_PASTE)
						{
							...
							sui_text_edit_paste(t, 256, &seduce_text_state[i].select_start, &seduce_text_state[i].select_end, NULL, 0);
							for(k = 0; t[k] != 0; k++)
							{
								if(t[k] == '-')
									negative = TRUE;
								...
								{
									l = k;
									sui_text_edit_delete(t, 256, &k, &l, NULL, 0);
									k--;
							...
							{
								uint s = 0, e = 0;
								sui_text_edit_insert_character(t, 256, &s, &e, '-', NULL, 0);
								seduce_text_state[i].select_start++;
								seduce_text_state[i].select_end++;
							}
						}else if(input->button_event[j].button == BETRAY_BUTTON_CUT)
							sui_text_edit_cut(t, 256, &seduce_text_state[i].select_start, &seduce_text_state[i].select_end, NULL, 0);
						else if(input->button_event[j].button == BETRAY_BUTTON_COPY)
							...
							sui_text_edit_delete(t, 256, &seduce_text_state[i].select_start, &seduce_text_state[i].select_end, NULL, 0);
						else if(input->button_event[j].button == BETRAY_BUTTON_END)
							...
							sui_text_edit_backspace(t, 256, &seduce_text_state[i].select_start, &seduce_text_state[i].select_end, NULL, 0);
						else if(input->button_event[j].button == BETRAY_BUTTON_RETURN) 
						{
							...
							sui_text_edit_insert_character(t, 256, &seduce_text_state[i].select_start, &seduce_text_state[i].select_end, input->button_event[j].character, NULL, 0);
						...
						{
							uint s = 0, e = 0;
							sui_text_edit_delete(t, 256, &s, &e, NULL, 0);
							...
							input->button_event[j].character == ',')
						{
							...
							{
								l = k;
								sui_text_edit_delete(t, 256, &l, &k, NULL, 0);
								if(seduce_text_state[i].select_start >= l)
									seduce_text_state[i].select_start--;
								if(seduce_text_state[i].select_end >= l)
									seduce_text_state[i].select_end--;
							}
							sui_text_edit_insert_character(t, 256, &seduce_text_state[i].select_start, &seduce_text_state[i].select_end, '.', NULL, 0);
						}
					}
				}
			}
