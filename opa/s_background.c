...
float seduce_color_settings_array[SEDUCE_CS_COUNT * 4] = {0.1, 0.1, 0.1, 0.1,
													0.4, 0.4, 0.3, 0,
													0.25, 0.5, 0.4, 0};

void seduce_color_set(SColorSettings setting, float red, float green, float blue, float alpha)
{
	if(setting < SEDUCE_CS_COUNT)
	{
		seduce_color_settings_array[setting * 4 + 0] = red;
		seduce_color_settings_array[setting * 4 + 1] = green;
		seduce_color_settings_array[setting * 4 + 2] = blue;
		seduce_color_settings_array[setting * 4 + 3] = alpha;
	}
}
...
void seduce_background_particle_init()
{
	...
	SBackgroundRender.light_texture = r_texture_allocate(R_IF_RGB_FLOAT16, 256, 256, 1, TRUE, FALSE, NULL);
	//	particle_debug_texture_id = SBackgroundRender.light_texture;
	
	SBackgroundRender.particle_fbo = r_framebuffer_allocate(&SBackgroundRender.particle_texture, 1, -1);
	SBackgroundRender.light_fbo = r_framebuffer_allocate(&SBackgroundRender.light_texture, 1, -1);
	...
	//	glBlendFunc(GL_ONE, GL_ONE);
		...
		for(i = 0; i < input->pointer_count; i++)
				r_primitive_image(input->pointers[i].pointer_x - 0.5, 
								(input->pointers[i].pointer_y - 0.5) / aspect, -1.0, 1, 1 / aspect, 0, 0, 1, 1, SBackgroundRender.sprite_texture, rgb[0], rgb[1], rgb[2], 0);
			...
			if(SBackgroundRender.particles[i].age < 1.0)
			{
				switch(SBackgroundRender.particles[i].type)	
				{
					...
					r_primitive_image(SBackgroundRender.particles[i].pos[0] - 0.25, 
								(SBackgroundRender.particles[i].pos[1] - 0.25) / aspect, -1.0, 0.5, 0.5 / aspect, 0, 0, 1, 1, SBackgroundRender.sprite_texture, f_randf(i) * f, f_randf(i + 1) * f, f_randf(i + 2) * f, f);
					break;
					case S_PT_CLICK :
...
boolean seduce_background_angle(BInputState *input, void *id, uint part, float pos_x, float pos_y, float angle_a, float angle_b, float timer)
{
	uint i, j;
	if(input->mode == BAM_DRAW)
	{
...
boolean seduce_background_gallery_vertical_draw(BInputState *input, float pos_x, float pos_y, float pos_z, float size_x, float *size_y, float image_height, uint *texture_id, float *aspects, uint image_count, float timer, float *center)
{
	...
	}else if(input->mode == BAM_EVENT)
	{
		...
		for(i = 0; i < input->pointer_count; i++)
			if(input->pointers[i].button[0] && input->pointers[i].last_button[0])
				if(&texture_id[i] == seduce_element_pointer_id(input, i, NULL))
					return TRUE;
	}
    return TRUE;
}
...
boolean seduce_background_gallery_horizontal_draw(BInputState *input, float pos_x, float pos_y, float pos_z, float *size_x, float size_y, float image_height, uint *texture_id, float *aspects, uint image_count, float timer, float *center)
{
