...
extern void 	*r_framebuffer_allocate(uint *buffers, uint buffer_count, uint depth_buffer); /* Creates a FBO form an array of textures and/or a depth/stencil buffer. All buffers has to be of the same size. it will return -1 if the different texture and depth formats are not renderable or compatible.*/
