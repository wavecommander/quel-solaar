...
uint f_text_parce_real(char *text, uint64 *integer_output, double *real_output, boolean *decimal)
{
	uint64 out = 0;
	boolean comma = FALSE;
	double divider = 0.1;
	...
	*integer_output = 0;
	...
	{
		*decimal = FALSE;
		i = f_text_parce_hex(&text[2], integer_output);
...
void opa_tokenizer_keywords(OPATokenFile *tokens)
{
	uint64 number = 0;
	OPAToken *t, *prev, *next;
	uint i, j, k;
	...
	if(i < tokens->token_count)
	{
		for(t = &tokens->tokens[i]; t != NULL; t = t->next)
		{
			if(t->type == OPA_TT_NAME)
			{
				for(j = 0; j < 32; j++)
				{
					for(k = 0; opa_languange_keywords[j][k] != 0 && opa_languange_keywords[j][k] == tokens->buffer[t->start + k]; k++);
					if(opa_languange_keywords[j][k] == 0 && t->length == k)
					{
						t->type = OPA_TT_KEYWORD_AUTO + j;
						break;
					}
				}
			}		
