...
extern THandle *testify_network_stream_wait_for_connection(THandle *listen, UnravelAddress *from); /*Polls a TCP handle "listen" for incomming connections. If a new connection is obtained a new handle for that connection will be returned. If the pointer from is given, the function will fill out the structure with the IP address and port of the incomming connection if one is obtained.*/
...
extern boolean	testify_network_address_lookup(UnravelAddress *address, char *dns_name, uint16 default_port); /* look up a DNS address and turn it in to a IP and port. Returns TRUE if sucessfull. The default port will be set unless the dns_name contains a port in the following format hostname:portnumber .*/
...
extern int		testify_network_datagram_send(THandle *handle, UnravelAddress *to); /* Sends the data collected in handle to the IP and address specifyed in "to".*/
extern int		testify_network_datagram_receive(THandle *handle, UnravelAddress *from); /*Querys for incoomming UDP packets. Returns 0  if no packets have arrived. If data has arrived the length of data will return and the "from" pointer will be filled out with the IP and port of the sender.*/
...
/* ---------- LAN discovery API --------
API for obtaining the addresses of all computers on the local area network*/

extern uint		testify_network_lan_count(); /* Retrive the number of possibhle hosts on local area network. */
extern boolean  testify_network_lan_address_get(uint id, char *buffer, uint buffer_length); /* Fills in an address of a possible client on the local network. Returns TRUE if the id is below the retuning value of testify_network_lan_count() and buffer_length is large enough to store the address filled out in buffer. */
