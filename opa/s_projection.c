...
void seduce_view_rotate(SViewData *v, BInputState *input, float x, float y)
{
	...
	if(v->type != S_VM_ROTATE)
	{
		...
		v->data.rotate.slide = TRUE;
		v->type = S_VM_ROTATE;
	}

	v->data.rotate.delta_rot[0] = v->data.rotate.delta_rot[0] * 0.5 + x * 1.0 / input->delta_time;
	v->data.rotate.delta_rot[1] = v->data.rotate.delta_rot[1] * 0.5 + y * 1.0 / input->delta_time;
}
		...
		for(i = 0; i < count; i++)
		{
			...
			if(users[i].count == 2 || users[i].count == 3)
			{
				...
				if(users[input->pointers[i].user_id].distance < 100.0)
					seduce_view_distance_matrix(v, dist / users[i].distance * v->distance);
