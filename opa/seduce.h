...
Seduce has a comprehencive thext and rendeing system. Seduce comes with a number of built in fonts, and you can load more .pff (Polygon font format) fonts by simply placing them in the same directory as the application. New .pff files can be generated from true type fonts using the Seduce font converter. The font system supports advanced kerning, and all UTF-8 characters.*/

#ifndef SEDUCE_INTERNAL
typedef void SeduceRenderFont; /* A handle prpresenting a loaded Seduce font */
#endif
...
extern float	seduce_text_line_draw(SeduceRenderFont *font, float pos_x, float pos_y, float letter_size, float letter_spacing, const char *text, float red, float green, float blue, float alpha, uint length);
...
extern boolean seduce_text_active(void *id); /* Returns True if the id is curtrently active in one of the text editing functions */
...
extern uint		seduce_text_block_draw(float pos_x, float pos_y, float line_size, float height,  float line_spacing, SUIBlockStyle style, const char *text, uint text_pos, STextBlockMode *modes, uint mode_count);
...
extern float	seduce_text_block_height(float line_size, float line_spacing, SUIBlockStyle style, const char *text, uint pos, STextBlockMode *modes, uint mode_count, uint end);

//extern uint	seduce_text_block_draw_old(float pos_x, float pos_y, float letter_size, float letter_spacing, float line_size, float line_spacing, uint line_count, SUIBlockStyle style, const char *text, uint text_start, float *color, uint *color_switch, uint *scroll, void (*func)(float pos_x, float pos_y, float *start, float *end, void *user), void *user);
...
extern STypeInState seduce_text_box_edit(BInputState *input, void *id, char *text, uint buffer_size, STextBox *boxes, uint box_count, STextBlockMode *modes, uint mode_count);
extern STypeInState seduce_text_monospace_edit(BInputState *input, void *id, char *text, uint buffer_size, void *font, float pos_x, float pos_y, float character_size, float space_size, float line_size,  uint line_count, uint line_length, uint *scroll, STextBlockMode *modes, uint mode_count);
...
extern void seduce_object_3d_draw(BInputState *input, float pos_x, float pos_y, float pos_z, float size, uint id, float fade, float *color);
extern uint seduce_object_3d_count();
extern char *seduce_object_3d_object_name(uint object);
extern uint seduce_object_3d_object_lookup(char *name);
...
extern STypeInState seduce_widget_select_radial(BInputState *input, void *id, uint *selected, char **lables, uint element_count, SPopUpType type, float pos_x, float pos_y, float size, float scale, float time, boolean release_only);


extern STypeInState seduce_select_draw(BInputState *input, void *id, uint user, float red, float green, float blue);
extern boolean seduce_select_query_pos(BInputState *input, uint user_id, float x, float y);
extern boolean seduce_select_query_id(BInputState *input, uint user_id, void *id);
...
extern void seduce_element_add_rectangle(BInputState *input, void *id, uint part, float pos_x, float pos_y, float size_x, float size_y);
...
extern void *seduce_element_pointer_down_click_id(BInputState *input, uint pointer, uint *part);
...
extern boolean seduce_element_surface_project(BInputState *input, void *id, float *output, uint axis, float pointer_x, float pointer_y);
extern boolean seduce_element_active(BInputState *input, void *id, void *part);

extern boolean seduce_element_position_get(void *id, float *pos);
