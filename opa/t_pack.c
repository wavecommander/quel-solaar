...
/*	if(handle->read_buffer_pos + testify_type_size[type] > testify_buffer_size[handle->type] && handle->type == T_HT_PACKET_PEER)
	...
	if(handle->debug_descriptor)
	{
		...
		{
			printf("UNRAVEL Error: Trying to read data that isnt there in file %s on line %u %u %u\n", file, line, handle->read_buffer_pos + 5, handle->read_buffer_used);
			exit(0);
		}
		...
		{
			printf("UNRAVEL read error on line %u in file: %s (%u)\n", line, file, handle->read_buffer_pos);
		...
		{
			uint j;
			printf("UNRAVEL read error on line %u in file: %s\n", line, file);
			for(j = 0; j < 22; j++)
...
#endif
	/* write out the data */
	data = ((int32) handle->read_buffer[handle->read_buffer_pos++]) << 56;
	data = ((int32) handle->read_buffer[handle->read_buffer_pos++]) << 48;
	data = ((int32) handle->read_buffer[handle->read_buffer_pos++]) << 40;
	data = ((int32) handle->read_buffer[handle->read_buffer_pos++]) << 32;
	data = ((int32) handle->read_buffer[handle->read_buffer_pos++]) << 24;
	data |= ((int32) handle->read_buffer[handle->read_buffer_pos++]) << 16;
	data |= ((int32) handle->read_buffer[handle->read_buffer_pos++]) << 8;
	data |= (int32) handle->read_buffer[handle->read_buffer_pos++];
	...
	if(handle->debug_descriptor)
	{
		...
		while(pos + 5 < handle->read_buffer_used && size > colected)
		{
			...
			if(pos == handle->read_buffer_used)
				return FALSE;
			if(read_type < T_TYPE_UINT8_VECTOR)
				add = testify_type_size[read_type];
			else
				add = testify_type_size[read_type % T_TYPE_UINT8_VECTOR] * length;
			pos += add;
			if(pos > handle->read_buffer_used)
				return FALSE;
			colected += add;
