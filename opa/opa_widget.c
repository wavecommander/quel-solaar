...
#define opa_number_draw_macro(data_type, value_type, function_name, spacing) \
				{ \
					...
					{ \
						...
						state = function_name(input, &pointer[i + options->scroll], &value, x + SEDUCE_T_SIZE + (float)(i % options->columns) * spacing, y - (float)(i / options->columns) * SEDUCE_T_SIZE * 4.0,  0.1, SEDUCE_T_SIZE * 2, TRUE, NULL, NULL, 0.7, 0.7, 0.7, 1, 1, 1, 1, 1); \
...
{	
	...
	if(array_length > options->columns)
	{
		...
		if(seduce_widget_icon_button_invisible(input, &options->expand, x + options->width * (float)options->columns * 0.5, y + SEDUCE_T_SIZE * 2, 0.001))
...
//		array_length = columns;
			...
			for(j = 0; j < draw_length; j++)
			{
				...
				if(*pointer == 0)
				{
					...
					}else
					{
						STypeInState state;
						static uint value;
						state =	seduce_text_uint_edit(input, pointer, &value, x + SEDUCE_T_SIZE, y,  0.1, SEDUCE_T_SIZE * 2, TRUE, NULL, NULL, 0.7, 0.7, 0.7, 1, 1, 1, 1, 1);
	...
	}else
	{
				...
				for(i = 0; i < draw_length; i++)
				{
					for(j = 0; j < project->types[type].member_count; j++)
					{
						...
						}else
						{
							if(input->mode == BAM_DRAW)
							{
								seduce_text_line_draw(NULL, -0.05 - x - seduce_text_line_length(SEDUCE_T_SIZE * 2, SEDUCE_T_SPACE, project->types[type].members[j].value_name, -1) - SEDUCE_T_SIZE, y, SEDUCE_T_SIZE * 2, SEDUCE_T_SPACE, project->types[type].members[j].value_name, 0.5, 0.7, 0.0, 1, -1);
								if(x > -0.01)
								{
		...
		}else switch(type)
		{
			...
			case OPA_TYPE_SINGED_CHAR :
			{
				...
				if(i < array_length)
				{
					if(array_length > 256 && options->expand)
					{
						...
						size = seduce_text_block_height(0.6, 2, SEDUCE_BS_LEFT, pointer, 0, &block, 1, -1);
						box.pos_x = x;
					...
					{
						if(S_TIS_IDLE != seduce_text_line_edit(input, pointer, pointer, array_length, x + SEDUCE_T_SIZE, y, 0.4, SEDUCE_T_SIZE * 2, "Terminated string", TRUE, NULL, NULL, 0.7, 0.7, 0.7, 1, 1, 1, 1, 1))
						{
							update_start = offset;
							update_length = array_length;
						}
				...
				}else
				{
					...
					opa_number_draw_macro(char, int, seduce_text_int_edit, options->width);
			...
			{
				y = opa_widget_draw_grid(input, x, y, draw_length, 0.1, options, &memmory[offset]);
				opa_number_draw_macro(unsigned char, uint, seduce_text_int_edit, options->width);
				y = opa_widget_draw_expand(input, x, y, SEDUCE_T_SIZE * 4 * ((draw_length + options->columns - 1) / options->columns), array_length, draw_length, options);
			}
			break;
			case OPA_TYPE_SIGNED_SHORT :
			{
				...
				y = opa_widget_draw_grid(input, x, y, draw_length, 0.1, options, &memmory[offset]);
				opa_number_draw_macro(short, int, seduce_text_int_edit, options->width);
				y = opa_widget_draw_expand(input, x, y, SEDUCE_T_SIZE * 4 * ((draw_length + options->columns - 1) / options->columns), array_length, draw_length, options);
			}
			break;
			...
			{
				y = opa_widget_draw_grid(input, x, y, draw_length, 0.1, options, &memmory[offset]);
				opa_number_draw_macro(unsigned short, uint, seduce_text_uint_edit, options->width);
				y = opa_widget_draw_expand(input, x, y, SEDUCE_T_SIZE * 4 * ((draw_length + options->columns - 1) / options->columns), array_length, draw_length, options);
			}
			break;
			...
			{
				y = opa_widget_draw_grid(input, x, y, draw_length, 0.1, options, &memmory[offset]);
				opa_number_draw_macro(int, int, seduce_text_int_edit, options->width);
				y = opa_widget_draw_expand(input, x, y, SEDUCE_T_SIZE * 4 * ((draw_length + options->columns - 1) / options->columns), array_length, draw_length, options);
			}
			break;
			...
			{
				y = opa_widget_draw_grid(input, x, y, draw_length, 0.1, options, &memmory[offset]);
				opa_number_draw_macro(unsigned int, uint, seduce_text_uint_edit, options->width);
				y = opa_widget_draw_expand(input, x, y, SEDUCE_T_SIZE * 4 * ((draw_length + options->columns - 1) / options->columns), array_length, draw_length, options);
			}
			break;
			...
			{
				y = opa_widget_draw_grid(input, x, y, draw_length, 0.1, options, &memmory[offset]);
				opa_number_draw_macro(long long, int, seduce_text_int_edit, options->width);
				y = opa_widget_draw_expand(input, x, y, SEDUCE_T_SIZE * 4 * ((draw_length + options->columns - 1) / options->columns), array_length, draw_length, options);
			}
			break;
			...
			{
				y = opa_widget_draw_grid(input, x, y, draw_length, 0.1, options, &memmory[offset]);
				opa_number_draw_macro(unsigned long long, uint, seduce_text_uint_edit, options->width);
				y = opa_widget_draw_expand(input, x, y, SEDUCE_T_SIZE * 4 * ((draw_length + options->columns - 1) / options->columns), array_length, draw_length, options);
			}
			break;
			case OPA_TYPE_FLOAT :
			{
				...
				opa_number_draw_macro(float, float, seduce_text_float_edit, options->width);
				y = opa_widget_draw_expand(input, x, y, SEDUCE_T_SIZE * 4 * ((draw_length + options->columns - 1) / options->columns), array_length, draw_length, options);
			}
			break;
			...
			{
				y = opa_widget_draw_grid(input, x, y, draw_length, 0.2, options, &memmory[offset]);
				opa_number_draw_macro(double, double, seduce_text_double_edit, options->width);
				y = opa_widget_draw_expand(input, x, y, SEDUCE_T_SIZE * 4 * ((draw_length + options->columns - 1) / options->columns), array_length, draw_length, options);
			}
			break;
		}
...
void opa_widget_draw_memory(BInputState *input, THandle *handle, OPAProject *project, OPAMemory *memmory, char *type_lables)
{
	...
	x += seduce_text_line_length(SEDUCE_T_SIZE, SEDUCE_T_SPACE, "Cast", -1) + SEDUCE_T_SIZE;
	if(input->mode == BAM_DRAW)
	...
	if(memmory->parent != -1)
	{
		if(seduce_text_button(input, &memmory->hidden, x, y, 0, SEDUCE_T_SIZE, SEDUCE_T_SPACE, "Hide", 0.8, 0.8, 0.8, 1, 1, 1, 1, 1))
			memmory->hidden = TRUE;
		x += seduce_text_line_length(SEDUCE_T_SIZE, SEDUCE_T_SPACE, "Hide", -1) + SEDUCE_T_SIZE;
		if(input->mode == BAM_DRAW)
			r_primitive_line_2d(x, -SEDUCE_T_SIZE * 0.0, x, SEDUCE_T_SIZE * 1.5, 0.3, 0.6, 0.0, 1.0);
		x += SEDUCE_T_SIZE;
	}
	...
	x += seduce_text_line_length(SEDUCE_T_SIZE, SEDUCE_T_SPACE, "Load", -1) + SEDUCE_T_SIZE;
	if(input->mode == BAM_DRAW)
		r_primitive_line_2d(x, -SEDUCE_T_SIZE * 0.0, x, SEDUCE_T_SIZE * 1.5, 0.3, 0.6, 0.0, 1.0);
	x += SEDUCE_T_SIZE;
	...
	x += seduce_text_line_length(SEDUCE_T_SIZE, SEDUCE_T_SPACE, "Save", -1) + SEDUCE_T_SIZE;
	if(input->mode == BAM_DRAW)
		r_primitive_line_2d(x, -SEDUCE_T_SIZE * 0.0, x, SEDUCE_T_SIZE * 1.5, 0.3, 0.6, 0.0, 1.0);
	x += SEDUCE_T_SIZE;
	...
	x += seduce_text_line_length(SEDUCE_T_SIZE, SEDUCE_T_SPACE, "Export", -1) + SEDUCE_T_SIZE;
	if(input->mode == BAM_DRAW)
		r_primitive_line_2d(x, -SEDUCE_T_SIZE * 0.0, x, SEDUCE_T_SIZE * 1.5, 0.3, 0.6, 0.0, 1.0);
	x += SEDUCE_T_SIZE;
	if(memmory->paused)
	{
		...
		x += seduce_text_line_length(SEDUCE_T_SIZE, SEDUCE_T_SPACE, "UnPause", -1) + SEDUCE_T_SIZE;
	}else
	{
		if(seduce_text_button(input, &memmory->paused, x, y, 0, SEDUCE_T_SIZE, SEDUCE_T_SPACE, "Pause", 0.8, 0.8, 0.8, 1, 1, 1, 1, 1))
			memmory->paused = TRUE;
		x += seduce_text_line_length(SEDUCE_T_SIZE, SEDUCE_T_SPACE, "Pause", -1) + SEDUCE_T_SIZE;
	}
...
//	seduce_text_button(input, memmory->matrix, x + SEDUCE_T_SIZE, y + SEDUCE_T_SIZE * 2, 0, SEDUCE_T_SIZE * 2.0, SEDUCE_T_SPACE, "]", 0.8, 0.8, 0.8, 1, 1, 1, 1, 1);
	...
	x += seduce_text_line_length(SEDUCE_T_SIZE * 2.0, SEDUCE_T_SPACE, "[ ", -1) + SEDUCE_T_SIZE * 0.5;
	...
	if(!memmory->options.expand && memmory->data_size / project->types[memmory->type].size_of > 1)
	{
		...
		if(!seduce_text_active(&ids[2])) 
			length = seduce_text_line_length(SEDUCE_T_SIZE * 4.0, SEDUCE_T_SPACE, name, -1);
		else
			length = 0.15;
		if(S_TIS_DONE == seduce_text_uint_edit(input, &ids[2], &memmory->options.scroll, x, y + SEDUCE_T_SIZE * 4, length, SEDUCE_T_SIZE * 4.0, 0, NULL, NULL, 0.8, 0.8, 0.8, 1, 1, 1, 1, 1))
		{
	...
	sprintf(name, "%u", size);
	if(!seduce_text_active(&ids[1])) 
		length = seduce_text_line_length(SEDUCE_T_SIZE * 4.0, SEDUCE_T_SPACE, name, -1);
	else
		length = 0.15;
	if(S_TIS_DONE == seduce_text_uint_edit(input, &ids[1], &size, x, y + SEDUCE_T_SIZE * 4, length, SEDUCE_T_SIZE * 4.0, 0, NULL, NULL, 0.8, 0.8, 0.8, 1, 1, 1, 1, 1))
	{
