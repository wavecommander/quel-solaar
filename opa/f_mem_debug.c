...
boolean f_debug_mem_remove(void *buf, char *file, uint line, boolean realloc)
{
	STMemFreeBuf  *f;
	uint i, j, k;
...
void f_debug_mem_free(void *buf, char *file, uint line)
{
	...
	if(f_alloc_mutex != NULL)
		f_alloc_mutex_lock(f_alloc_mutex);
	if(!f_debug_mem_remove(buf, file, line, FALSE))
	{
		uint *X = NULL;
		X[0] = 0;
	}
...
void *f_debug_mem_realloc(void *pointer, uint size, char *file, uint line)
{
	...
	f_debug_mem_remove(pointer, file, line, TRUE);
	free(pointer);
