...
void betray_requester_save(char **types, uint type_count, void *id)
{
	if(b_win32_requester_mutex == NULL)
		b_win32_requester_mutex = imagine_mutex_create();
	imagine_thread(betray_requester_save_func, betray_requester_string_build(types, type_count, id));
}
...
void betray_requester_load(char **types, uint type_count, void *id)
{
	if(b_win32_requester_mutex == NULL)
		b_win32_requester_mutex = imagine_mutex_create();
	imagine_thread(betray_requester_load_func, betray_requester_string_build(types, type_count, id));
}
...
void betray_url_launch(char *url)
{
	...
	for(i = 0; url[i] != 0; i++)
		unicode[i] = url[i];
	unicode[i] = 0;
	imagine_thread(betray_url_launch_func, unicode);
}
