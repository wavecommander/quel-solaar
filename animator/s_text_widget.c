...
void sui_draw_text(float pos_x, float pos_y, float size, float spacing, const char *text, float red, float green, float blue, float alpha)
{
	...
	glColor4f(red, green, blue, 0.15 * alpha);
	sui_text_line_draw(size, spacing, text, -1, 10);
	glPopMatrix();
}
		...
		if(sui_type_in_text == text)
		{
			for(i = 0; i < input->event_count; i++)
			{
				if(input->event[i].state == TRUE)
				{
					...
					else if(input->event[i].character > 0 && input->event[i].character < 128)
...
boolean sui_type_number_double(BInputState *input, float pos_x, float pos_y, float center, float length, float size, double *number, void *id, float red, float green, float blue, float alpha)
{
	...
	if(input->mode == BAM_DRAW)
	{
		...
		{
			move = sui_text_line_length(size, SUI_T_SPACE, sui_type_in_copy, -1) * center;
			sui_draw_text(pos_x - move, pos_y, size, SUI_T_SPACE, sui_type_in_copy, red, green, blue, alpha);
			sui_text_line_edit_draw(pos_x - move, pos_y, size, SUI_T_SPACE, length, sui_type_in_copy, &scroll_start, select_start, select_end);
		}else
		{
			char nr[64];
			sprintf(nr, "%.4f", *number);
			move = sui_text_line_length(size, SUI_T_SPACE, nr, -1) * center;
			sui_draw_text(pos_x - move, pos_y, size, SUI_T_SPACE, nr, red, green, blue, alpha);
		}
	}else
	{
		if(input->mouse_button[0] == TRUE && input->last_mouse_button[0] == FALSE)
