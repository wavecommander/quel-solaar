...
void sui_draw_popup_square(float pos_x, float pos_y, float size_x, float size_y, float color)
{
	...
	sui_draw_gl(GL_QUADS, array, 4, 2, color, color, color, 0.2);
}
...
uint sui_draw_popup(BInputState *input, float pos_x, float pos_y, SUIPUElement *element, uint element_count, uint button, float back_color)
{
	...
	if(input->mode == BAM_DRAW)
	{
		...
		for(i = 0; i < element_count; i++)
			if(element[i].type == PU_T_TOP)
				top++;
		sui_draw_popup_square(pos_x - 0.125, pos_y + 0.125, 0.25, 0.05 * (float)(top), 1 - back_color);
		top = 0;

		for(i = 0; i < element_count; i++)
			if(element[i].type == PU_T_BOTTOM)
				bottom++;
		sui_draw_popup_square(pos_x - 0.125, pos_y - 0.125, 0.25, -0.05 * (float)(bottom), 1 - back_color);
		bottom = 0;
		...
		for(i = 0; i < element_count; i++)
		{
			...
			{
				case PU_T_TOP :
					if(sui_box_click_test(pos_x - 0.125, pos_y + (float)(top) * 0.05 + 0.125, 0.25, 0.05))
						sui_draw_popup_square(pos_x - 0.125, pos_y + (float)(top) * 0.05 + 0.175, 0.25, -0.05, 1 - back_color);
					sui_draw_text(pos_x + sui_text_line_length(SUI_T_SIZE, SUI_T_SPACE, element[i].text, -1) * -0.5, pos_y + (float)(top++) * 0.05 + 0.15 - SUI_T_SIZE, SUI_T_SIZE, SUI_T_SPACE, element[i].text, back_color, back_color, back_color, 1.0);
				break;
				case PU_T_BOTTOM :
					if(sui_box_click_test(pos_x - 0.125, pos_y + (float)(bottom) * -0.05 - 0.175, 0.25, 0.05))
						sui_draw_popup_square(pos_x - 0.125, pos_y + (float)(bottom) * -0.05 - 0.125, 0.25, -0.05, 1 - back_color);
					sui_draw_text(pos_x + sui_text_line_length(SUI_T_SIZE, SUI_T_SPACE, element[i].text, -1) * -0.5, pos_y + (float)(bottom++) * -0.05 - 0.15 - SUI_T_SIZE, SUI_T_SIZE, SUI_T_SPACE, element[i].text, back_color, back_color, back_color, 1.0);
				break;
				case PU_T_ANGLE :
					for(j = 0; j < i; j++)
					...
					{
						sui_draw_text(pos_x + sui_text_line_length(SUI_T_SIZE, SUI_T_SPACE, element[i].text, -1) * -0.5, pos_y + 0.15, SUI_T_SIZE, SUI_T_SPACE, element[i].text, back_color, back_color, back_color, 1.0);
					...
					}else if(element[i].data.angle[0] + element[i].data.angle[1] > 360)
					{
						...
						sui_draw_text(-0.15 - sui_text_line_length(SUI_T_SIZE, SUI_T_SPACE, element[i].text, -1), -SUI_T_SIZE, SUI_T_SIZE, SUI_T_SPACE, element[i].text, back_color, back_color, back_color, 1.0);
						glPopMatrix();
					}
					break;
				...
				case PU_T_VSLIDER :
					break;
				case PU_T_HSLIDER :
					sui_draw_text(pos_x + element[i].data.slider.pos[0] + (element[i].data.slider.length - sui_text_line_length(SUI_T_SIZE, SUI_T_SPACE, element[i].text, -1)) * 0.5, pos_y + element[i].data.slider.pos[1] + SUI_T_SIZE, SUI_T_SIZE, SUI_T_SPACE, element[i].text, back_color, back_color, back_color, 1.0);
	
					pos[0] = pos_x + element[i].data.slider.pos[0] + element[i].data.slider.length * *element[i].data.slider.value;
					pos[1] = pos_y + element[i].data.slider.pos[1];

					if(element[i].data.slider.length * *element[i].data.slider.value - 0.02 > 0)
						sui_draw_2d_line_gl(pos_x + element[i].data.slider.pos[0] + element[i].data.slider.length * *element[i].data.slider.value - 0.02,
											pos_y + element[i].data.slider.pos[1],
											pos_x + element[i].data.slider.pos[0],
											pos_y + element[i].data.slider.pos[1], 0.5, 0.5, 0.5, 0.5);

					if(element[i].data.slider.length * *element[i].data.slider.value + 0.02 < element[i].data.slider.length)
						sui_draw_2d_line_gl(pos_x + element[i].data.slider.pos[0] + element[i].data.slider.length * *element[i].data.slider.value + 0.02,
											pos_y + element[i].data.slider.pos[1],
											pos_x + element[i].data.slider.pos[0] + element[i].data.slider.length,
											pos_y + element[i].data.slider.pos[1], 0.5, 0.5, 0.5, 0.5);

					sui_manipulator_point_draw(pos[0], pos[1], 1.0, 1, 1, 1);
					break;
				case PU_T_SQUARE :
					...
					sui_draw_text(pos_x + element[i].data.square.square[0] + element[i].data.square.square[2] * 0.5 - sui_text_line_length(SUI_T_SIZE, SUI_T_SPACE, element[i].text, -1) * 0.5, -SUI_T_SIZE + pos_y + element[i].data.square.square[1] + element[i].data.square.square[3] * 0.5, SUI_T_SIZE, SUI_T_SPACE, element[i].text, back_color, back_color, back_color, 1.0);
					break;
				case PU_T_ICON :
	...
	}else if(input->mouse_button[0])
	{
		for(i = 0; i < element_count; i++)
		{
			...
			{
				case PU_T_HSLIDER :
					if(input->click_pointer_y < pos_y + element[i].data.slider.pos[1] + 0.03 &&
						input->click_pointer_y > pos_y + element[i].data.slider.pos[1] - 0.01 &&
						input->click_pointer_x > pos_x + element[i].data.slider.pos[0] &&
						input->click_pointer_x < pos_x + element[i].data.slider.pos[0] + element[i].data.slider.length)
					{
						*element[i].data.slider.value = (input->pointer_x - (pos_x + element[i].data.slider.pos[0])) / element[i].data.slider.length;
						if(*element[i].data.slider.value > 1.0)
							*element[i].data.slider.value  = 1.0;
						if(*element[i].data.slider.value < 0.0)
							*element[i].data.slider.value  = 0.0;
					}
				break;
			}
		}
