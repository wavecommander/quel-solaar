...
float c_character_spine_base(CCharacter *c, float *base, float *look, float *pos, float *upp, float *walk, float *look_dir, float delta)
{
	...
	{
		base[8] = base[8] * (1.0 - delta) - walk[0] * delta * 0.8 + look_dir[0] * delta * 0.2;
		base[9] = base[9] * (1.0 - delta) - walk[1] * delta * 0.8 + look_dir[1] * delta * 0.2;	
		base[10] = base[10] * (1.0 - delta) - walk[2] * delta * 0.8 + look_dir[2] * delta * 0.2;
	}else
	{
		base[8] = base[8] * (1.0 - delta) + walk[0] * delta * 0.8 + look_dir[0] * delta * 0.2;
		base[9] = base[9] * (1.0 - delta) + walk[1] * delta * 0.8 + look_dir[1] * delta * 0.2;
