...
void sui_manipulator_cross_draw(float pos_x, float pos_y, float red, float green, float blue)
{
	static float center_vertex[5 * 4 * 2], vertex_shadow[12 * 4 * 2], vertex_color[12 * 4 * 4];
	static boolean init = FALSE;
	if(!init)
	{
		float square[3 * 4 * 2];
		...
		square[22] = -0.005;
		square[23] = 0.01;
		sui_create_shadow_edge(0.01, 12, vertex_shadow, vertex_color, square);
		center_vertex[0] = 0.0;
		center_vertex[1] = 0.005;
		center_vertex[2] = 0.005;
		center_vertex[3] = 0.0;
		center_vertex[4] = 0.0;
		center_vertex[5] = -0.005;
		center_vertex[6] = -0.005;
		center_vertex[7] = 0.0;
		center_vertex[8] = 0.005;
		center_vertex[9] = 0.01;
		center_vertex[10] = 0.01;
		center_vertex[11] = 0.005;
		center_vertex[12] = 0.005;
		center_vertex[13] = 0.0;
		center_vertex[14] = 0.0;
		center_vertex[15] = 0.005;
		center_vertex[16] = -0.005;
		center_vertex[17] = 0.0;
		center_vertex[18] = 0.0;
		center_vertex[19] = -0.005;
		center_vertex[20] = -0.005;
		center_vertex[21] = -0.01;
		center_vertex[22] = -0.01;
		center_vertex[23] = -0.005;
		center_vertex[24] = 0.005;
		center_vertex[25] = 0.0;
		center_vertex[26] = 0.01;
		center_vertex[27] = -0.005;
		center_vertex[28] = 0.005;
		center_vertex[29] = -0.01;
		center_vertex[30] = 0.0;
		center_vertex[31] = -0.005;
		center_vertex[32] = -0.005;
		center_vertex[33] = 0.01;
		center_vertex[34] = 0.0;
		center_vertex[35] = 0.005;
		center_vertex[36] = -0.005;
		center_vertex[37] = 0.0;
		center_vertex[38] = -0.01;
		center_vertex[39] = 0.005;
		init = TRUE;
	}
	glPushMatrix();
	glTranslated(pos_x, pos_y, 0);
	sui_draw_gl(GL_QUADS, center_vertex, 20, 2, red, green, blue, 0.8);
	sui_set_color_array_gl(vertex_color, 12 * 4, 4);
	sui_draw_gl(GL_QUADS, vertex_shadow, 12 * 4, 2, red, green, blue, 0.8);
	glPopMatrix();
}

void sui_manipulator_point_draw(float pos_x, float pos_y, float scale, float red, float green, float blue)
{
...
boolean sui_manipulator_point(BInputState *input, SViewData *v, float *pos, void *id, float red, float green, float blue, char *name)
{
	...
	if(input->mode == BAM_DRAW)
	{
		...
		if(tmp[2] < 0)
		{
			sui_manipulator_point_draw(tmp[0], tmp[1], 1.0, red, green, blue);
			if(name != NULL)
			{
...
boolean sui_manipulator_normal(BInputState *input, SViewData *v, float *pos, float *normal, void *id, float red, float green, float blue)
{
	...
	if(input->mode == BAM_DRAW)
	{
		...
		sui_view_projection_screenf(v, tmp, pos[0] + normal[0] * dist, 
												pos[1] + normal[1] * dist, 
												pos[2] + normal[2] * dist);
		sui_manipulator_line_draw(place, tmp, 0.005, 0.005, red, green, blue);
		sui_manipulator_point_draw(tmp[0], tmp[1], 1.0,  red, green, blue);
	}
