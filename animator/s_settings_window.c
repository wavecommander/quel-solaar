...
double sui_draw_slider(BInputState *input, double x_pos, double y_pos, double width, double value, float *start_color, float *end_color, float *color)
{
	...
	if(input->mode == BAM_DRAW)
	{
		...
		line[0] = x_pos - 0.02;
		line[1] = y_pos;
		line[2] = x_pos - 0.02 + width * f;
		line[3] = y_pos;

		line[4] = x_pos + 0.02 + width * f;
		line[5] = y_pos;
		line[6] = x_pos + width + 0.02;
		line[7] = y_pos;

		sui_set_color_array_gl(col, 4, 3);
		sui_draw_gl(GL_LINES, line, 4, 2, 0, 0, 0, 1);
		pos[0] = x_pos + width * f;
		pos[1] = y_pos;
		pos[2] = 0;
		sui_manipulator_point_draw(pos[0], pos[1], 1.0, color[0], color[1], color[2]);
	}else
	{
		if(input->mouse_button[0] && input->click_pointer_x > x_pos && input->click_pointer_x < x_pos + width && y_pos - 0.0125 < input->click_pointer_y && y_pos + 0.0125 > input->click_pointer_y)
		{
...
{
	float back_color = 1.0, f, draw[168], shadow[(40 * 4 + 16) * 2], shadow_color[(40 * 4 + 16) * 4];

	width *= 0.5;
	compute_background(draw, shadow, shadow_color, radius, y_pos, y_pos - height, x_pos - width, x_pos + width);
	sui_set_color_array_gl(shadow_color, 40 * 4 + 16, 4);
	sui_draw_gl(GL_QUADS, shadow, 40 * 4 + 16, 2, 0.2, 0.5, 0.7, 1);
	sui_draw_gl(GL_QUADS, draw, 84, 2, red, green, blue, alpha);
	if(title != NULL)
	{
		if(red + green + blue > 1.5)
			back_color = 0.0;
		f = -0.5 * sui_text_line_length(SUI_T_SIZE, SUI_T_SPACE * 8, title, -1);
...
boolean sui_draw_setting_view(BInputState *input, double x_pos, double y_pos, double width, double scale, SUIViewElement *element, uint element_count, char *title, float back_color)
{
	...
	for(i = 0; i < element_count; i++)
		f += sui_setting_view_size(&element[i]) * scale;
	...
	if(input->mode == BAM_DRAW)
	{
		...
		f = -0.5 * sui_text_line_length(SUI_T_SIZE * scale, SUI_T_SPACE * 8, title, -1);
		sui_draw_text(x_pos + f, y_pos, SUI_T_SIZE * scale, SUI_T_SPACE * 8, title, color, color, color, 1);
		sui_draw_2d_line_gl(x_pos - f + 0.02, y_pos - 0.005 * scale, x_pos + f - 0.02, y_pos - 0.005 * scale, color, color, color, 1);
	}
	...
	{
		if(input->mode == BAM_DRAW)
			sui_draw_text(x_pos - width, y_pos, SUI_T_SIZE * scale, SUI_T_SPACE, element[i].text, color, color, color, 1);
		switch(element[i].type)
		{
			case S_VET_SPLIT :
				if(input->mode == BAM_DRAW)
					sui_draw_2d_line_gl(x_pos - width + 0.01 + sui_text_line_length(SUI_T_SIZE * scale, SUI_T_SPACE, element[i].text, -1), y_pos + 0.007, x_pos + width, y_pos + 0.007, color, color, color, 1);
			break;
			case S_VET_SPLIT_MULTI :
				{
					char nr[64];
					sprintf(nr, "<%u/%u>", element[i].param.split_multi.current + 1, element[i].param.split_multi.count);
					f = sui_text_line_length(SUI_T_SIZE * scale, SUI_T_SPACE, element[i].text, -1) + 0.03 - width;
					if(input->mode == BAM_DRAW)
					{
						sui_draw_2d_line_gl(x_pos + f - 0.01, y_pos + 0.007, x_pos + f - 0.02, y_pos + 0.007, color, color, color, 1);
						sui_draw_text(x_pos + f, y_pos, SUI_T_SIZE * scale, SUI_T_SPACE, nr, color, color, color, 1);
						f += sui_text_line_length(SUI_T_SIZE * scale, SUI_T_SPACE, nr, -1) + 0.03;
					}else
					{
						f2 = sui_text_line_length(SUI_T_SIZE * scale, SUI_T_SPACE, nr, -1);
					...
					{
						sui_draw_2d_line_gl(x_pos + f - 0.01, y_pos + 0.007, x_pos + f - 0.02, y_pos + 0.007, color, color, color, 1);
						sui_draw_text(x_pos + f, y_pos, SUI_T_SIZE * scale, SUI_T_SPACE, "Add Del", color, color, color, 1);
						f += sui_text_line_length(SUI_T_SIZE * scale, SUI_T_SPACE, "Add Del", -1) + 0.01;
					}else
					{
						f2 = sui_text_line_length(SUI_T_SIZE * scale, SUI_T_SPACE, "Add Del", -1);
						if(input->mouse_button[0] && !input->last_mouse_button[0])
						{
							if(input->pointer_x > x_pos + f && input->pointer_x < x_pos + f + f2 * 0.5 &&
			...
			case S_VET_BOOLEAN :
				if(input->mode == BAM_DRAW)
				{
					...
					{
						sui_manipulator_point_draw(pos[0], pos[1], 1.0, 0.9, 0.9, 0.9);
				//		sui_draw_2d_line_gl(x_pos + width - 0.02 * scale, y_pos + 0.01 * scale, x_pos + width, y_pos - 0.01 * scale, color, color, color, 1);
				//		sui_draw_2d_line_gl(x_pos + width - 0.02 * scale, y_pos - 0.01 * scale, x_pos + width, y_pos + 0.01 * scale, color, color, color, 1);
					}else
						sui_manipulator_point_draw(pos[0], pos[1], 1.0, 0.2, 0.2, 0.2);
				//	sui_draw_2d_line_gl(x_pos + width - 0.02 * scale, y_pos + 0.01 * scale, x_pos + width - 0.00 * scale, y_pos + 0.01 * scale, color, color, color, 1);
				//	sui_draw_2d_line_gl(x_pos + width - 0.02 * scale, y_pos - 0.01 * scale, x_pos + width - 0.00 * scale, y_pos - 0.01 * scale, color, color, color, 1);
				//	sui_draw_2d_line_gl(x_pos + width - 0.02 * scale, y_pos + 0.01 * scale, x_pos + width - 0.02 * scale, y_pos - 0.01 * scale, color, color, color, 1);
				//	sui_draw_2d_line_gl(x_pos + width - 0.00 * scale, y_pos + 0.01 * scale, x_pos + width - 0.00 * scale, y_pos - 0.01 * scale, color, color, color, 1);
			...
			break;
			case S_VET_INTEGER :
				sui_type_number_uint(input, x_pos + width, y_pos, 1, 0.1, SUI_T_SIZE * scale, &element[i].param.integer, &element[i].param.integer, color, color, color, color);
			break;
			case S_VET_REAL :
				sui_type_number_double(input, x_pos + width, y_pos, 1, 0.1, SUI_T_SIZE * scale, &element[i].param.real, &element[i].param.real, color, color, color, 1.0);
			break;
			case S_VET_SLIDER :
				{				
			...
			case S_VET_TEXT :
				...
				{
					sui_type_in(input, x_pos, y_pos, width, SUI_T_SIZE * scale, element[i].param.text.text, element[i].param.text.length, NULL, NULL, color, color, color, 1.0);
				}else
				{
					f = sui_text_line_length(SUI_T_SIZE * scale, SUI_T_SPACE, element[i].param.text.text, -1);
					sui_type_in(input, x_pos + width - f, y_pos, f, SUI_T_SIZE * scale, element[i].param.text.text, element[i].param.text.length, NULL, NULL, color, color, color, 1.0);
				}
			break;
			case S_VET_COLOR :
				{				
			...
			case S_VET_SELECT :
				...
				{
					f2 = f;
					f -= sui_text_line_length(SUI_T_SIZE * scale, SUI_T_SPACE, element[i].param.select.text[j - 1], -1);
					if(input->mode == BAM_DRAW)
					{
						sui_draw_text(x_pos + f, y_pos, SUI_T_SIZE * scale, SUI_T_SPACE, element[i].param.select.text[j - 1], color, color, color, 1);
						if(element[i].param.select.select == j - 1)
							sui_draw_2d_line_gl(x_pos + f - 0.005, y_pos - 0.02 * scale, x_pos + f2 + 0.005, y_pos - 0.02 * scale, color, color, color, 1);
					}else
					{
			...
			case S_VET_CUSTOM :
				...
				y_pos -= element[i].param.custom.length + 0.05 * scale;
			break;
		}
		y_pos -= 0.05 * scale;
	}
...
//	time += 0.1;
	...
	sui_draw_setting_view(input, sin(time) * 0.1, cos(time) * 0.1, 0.5, 1.0, element, 10, "TITLE", 1.0);
