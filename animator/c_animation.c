...
void c_character_from_matrix(float *output, CCharacter *c, CAnimPoint *point, uint value_id)
{
	float tmp[3], *matrix;
	if(point->space >= C_CM_COUNT)
	{
...
void c_character_from_matrix_animation(float *output, CCharacter *c, CAnimEvent *e, uint value_id, float time)
{
	...
	if(points->point_count < 2)
		c_character_from_matrix(output, c, points->points, value_id);
	else
	{
		...
		if(e->loop)
		{
			...
			time = (time - t0) / (t1 - t0);
			c_character_from_matrix(p0, c, &points->points[(i + points->point_count - 1) % points->point_count], value_id);
			c_character_from_matrix(p1, c, &points->points[i % points->point_count], value_id);
		...
		}else
		{
			...
			{
				c_character_from_matrix(output, c, &points->points[points->point_count - 1], value_id);
				return;
			}
			if(i != 0)
				t0 = points->points[i - 1].time;
			...
			{
				c_character_from_matrix(output, c, &points->points[0], value_id);
				return;
			}
			time = (time - t0) / (t1 - t0);
			c_character_from_matrix(p0, c, &points->points[(i + points->point_count - 1) % points->point_count], value_id);
...
{
	CAnimEvent *e;
	float t, default_pos[C_APC_COUNT][3], zero[3] = {0, 0, 0}, pos[3];
	uint i, j;
		...
		}else // moving towards the next tool
		{
			for(i = 0; i < C_APC_COUNT; i++)
			{
				...
				if(c->tool_current != -1 && desc->events[c->tool_current].anim[i].point_count > 0)
					...
					c_character_from_matrix(pos, c, &desc->default_pose[i], i);
	...
	}else
	{
		for(i = 0; i < C_APC_COUNT; i++)
		{
			c_character_from_matrix(c->points[i].pos, c, &desc->default_pose[i], i);
		}
