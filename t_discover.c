...
TestifyDiscovery *testify_discovery = NULL;
...
void testify_discover_user_remove(TestifyDiscovery *discover, char *user)
{
	uint i, j;
...
TestifyDiscovery *testify_discover_add(char *user, char *service, uint32 ip, uint16 port, char *message)
{
	uint8 buffer[513];
	uint i, j, k;
	for(i = 0; i < testify_discovery_count; i++)
...
TestifyDiscovery *testify_discover_parse_message(char *user, char *string)
{
	...
	if(buffer[j] == 4)
	{
		...
		for(i = 0; i < testify_network_adapter_count() && testify_network_adapter_ipv4_global_address(i) != ip; i++);
		if(i == testify_network_adapter_count())
		{
			buffer[j] = 0;
...
TestifyNetworkPeer *testify_discover(char *service, uint16 port, uint *count, uint8 *seed, uint seed_length, char *message, boolean global)
{
	...
	TestifyDiscovery *discovery = NULL;
	...
	if(udp_handle != NULL)
	{
		...
		{	
			...
			d = testify_discover_add(NULL, s, a.ip, reply_port, m, TRUE);
					...
					if(user_name[i] != 0) /* remove any message from me*/
 					{
						...
						if(command[i] == '#')
						{
							...
							if(i != -1 && ip_address != 0)
								testify_discover_pack_message(handle, &testify_discovery[i], name, ip_address, testify_discovery[i].port, testify_discovery[i].service);
						}
					}
				}
