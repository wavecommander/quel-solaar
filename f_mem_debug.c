...
boolean f_debug_memory()
{
	...
	for(i = 0; i < f_alloc_line_count && i < 0; i++)
	{
...
boolean f_debug_mem_remove(void *buf, char *file, uint line, boolean realloc, size_t *size)
{
	...
	for(i = 0; i < f_freed_memory_count; i++)
	{
		if(f != &f_freed_memory[i] && buf == f_freed_memory[i].pointer)
		{
			uint *a = NULL;
			if(f->realloc)
				printf("MEM ERROR: Pointer %p in file is beeng freed twice! if was freed one line %u in %s, was reallocated to %u bytes long one line %u in file %s\n", f->pointer, f->free_line, f->free_file, f->size, f->alloc_line, f->alloc_file);
			else
				printf("MEM ERROR: Pointer %p in file is beeng freed twice! if was freed one line %u in %s, was allocated to %u bytes long one line %u in file %s\n", f->pointer, f->free_line, f->free_file, f->size, f->alloc_line, f->alloc_file);
	//		a[0] = 0;
		}
	}
...
boolean f_debug_mem_test(void *pointer, size_t size, boolean ignore_not_found)
{
	...
	for(i = 0; i < f_alloc_line_count; i++)
	{
		for(j = 0; j < f_alloc_lines[i].alloc_count; j++)
		{
			...
			{
				if(((uint8 *)f_alloc_lines[i].allocs[j].buf) + f_alloc_lines[i].allocs[j].size < ((uint *)pointer) + size)
				{
					printf("MEM ERROR: Not enough memory to access, %u bytes missing\n",  (uint)(((uint8 *)f_alloc_lines[i].allocs[j].buf) + f_alloc_lines[i].allocs[j].size) - (uint)(((uint8 *)pointer) + size));
					if(f_alloc_mutex != NULL)
						f_alloc_mutex_unlock(f_alloc_mutex);
					return TRUE;
				}else
	...
	if(ignore_not_found)
		return FALSE;
	printf("MEM ERROR: No matching memory found!\n");
	return TRUE;
}
