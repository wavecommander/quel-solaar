...
struct{
	...
	uint			device_count;
	BButtonEvent	*held_buttons;
	uint			held_button_count;
	uint			held_button_allocated;
}BGlobal;
...
void betray_plugin_button_set(uint user_id, uint id, boolean state, uint character)
{
	...
	if(state)
	{
		if(BGlobal.held_button_count == BGlobal.held_button_allocated)
		{
			BGlobal.held_button_allocated += 16;
			BGlobal.held_buttons = realloc(BGlobal.held_buttons, (sizeof *BGlobal.held_buttons) * BGlobal.held_button_allocated);
		}
