...
boolean sui_draw_setting_view(BInputState *input, double x_pos, double y_pos, double width, SUIViewElement *element, uint element_count, char *title, float back_color)
{
	...
	for(i = 0; i < element_count; i++)
	{
		...
		switch(element[i].type)
		{
			case S_VET_SPLIT :
				if(input->mode == BAM_DRAW)
					sui_draw_2d_line_gl(x_pos - width + 0.01 + sui_text_line_length(SUI_T_SIZE, SUI_T_SPACE, element[i].text, -1), y_pos + 0.007, x_pos + width, y_pos + 0.007, color, color, color, 1);
			break;
			case S_VET_SPLIT_MULTI :
				{
					char nr[64];
					sprintf(nr, "<%u/%u>", element[i].param.split_multi.current + 1, element[i].param.split_multi.count);
					f = sui_text_line_length(SUI_T_SIZE, SUI_T_SPACE, element[i].text, -1) + 0.03 - width;
					if(input->mode == BAM_DRAW)
					{
						...
						f += sui_text_line_length(SUI_T_SIZE, SUI_T_SPACE, nr, -1) + 0.03;
					}else
					{
						f2 = sui_text_line_length(SUI_T_SIZE, SUI_T_SPACE, nr, -1);
					...
					if(input->mode == BAM_DRAW)
					{
						...
						f += sui_text_line_length(SUI_T_SIZE, SUI_T_SPACE, "Add Del", -1) + 0.01;
					}else
					{
						f2 = sui_text_line_length(SUI_T_SIZE, SUI_T_SPACE, "Add Del", -1);
						if(input->mouse_button[0] && !input->last_mouse_button[0])
						{
							if(input->pointer_x > x_pos + f && input->pointer_x < x_pos + f + f2 * 0.5 &&
			...
			break;
			case S_VET_INTEGER :
				sui_type_number_uint(input, x_pos + width, y_pos, 1, 0.1, SUI_T_SIZE, &element[i].param.integer, &element[i].param.integer, color, color, color, color);
			break;
			case S_VET_REAL :
			...
			case S_VET_TEXT :
				...
				{
					f = sui_text_line_length(SUI_T_SIZE, SUI_T_SPACE, element[i].param.text.text, -1);
			...
			case S_VET_SELECT :
				...
				{
					f2 = f;
					f -= sui_text_line_length(SUI_T_SIZE, SUI_T_SPACE, element[i].param.select.text[j - 1], -1);
					if(input->mode == BAM_DRAW)
					{
					...
					{
						if(input->mouse_button[0] && !input->last_mouse_button[0] && input->click_pointer_x > x_pos + f - 0.02 && input->click_pointer_x < x_pos + f2 + 0.02  && y_pos - 0.025 < input->click_pointer_y && y_pos + 0.025 > input->click_pointer_y)
						{
