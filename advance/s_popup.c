		...
		for(i = 0; i < element_count; i++)
		{
			switch(element[i].type)
			{
				case PU_T_TOP :
					...
					sui_draw_text(pos_x + sui_text_line_length(SUI_T_SIZE, SUI_T_SPACE, element[i].text, -1) * -0.5, pos_y + (float)(top++) * 0.05 + 0.1 - SUI_T_SIZE, SUI_T_SIZE, SUI_T_SPACE, element[i].text, back_color, back_color, back_color, 1.0);
				break;
				case PU_T_BOTTOM :
					...
					sui_draw_text(pos_x + sui_text_line_length(SUI_T_SIZE, SUI_T_SPACE, element[i].text, -1) * -0.5, pos_y + (float)(bottom++) * -0.05 - 0.1 - SUI_T_SIZE, SUI_T_SIZE, SUI_T_SPACE, element[i].text, back_color, back_color, back_color, 1.0);
				break;
				case PU_T_ANGLE :
					for(j = 0; j < i; j++)
